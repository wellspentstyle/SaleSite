...
- **Featured Sales Assets**: Generates 1080x1350 Instagram images from selected sales, auto-uploads to Google Drive. **Asset Configurator (Nov 2024)**: Click-to-configure workflow on Generate Assets page. Main sale image offers two options: "Without Picks" (header-only with brand name/discount) or "With Picks" (header + 3 product images with selectable picks). Individual pick stories (1080x1920) support optional custom copy that appears in a black bar at the top-right corner, matching the existing bottom-left styling. All generated assets upload to Google Drive with Airtable tracking (FeaturedAssetURL/FeaturedAssetDate fields). **Instagram Posting (Nov 2024)**: Integrated Late.dev API for programmatic Instagram posting. Asset Configurator includes "Post to Instagram" toggle that enables direct posting of generated assets to Instagram feed (main images) and stories (pick images). Features caption input with hashtag suggestions, separate toggles for feed posts vs stories, and gradient-styled "Generate & Post" button. Backend service (`server/instagram-poster.js`) supports single image posts, carousels, stories, and scheduled posts. Requires `LATE_API_KEY` and `LATE_INSTAGRAM_ACCOUNT_ID` secrets.
...
- **Late.dev**: Social media API for Instagram posting (posts, stories, carousels, scheduling).
