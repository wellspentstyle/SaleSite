...
- **Featured Sales Assets**: Generates 1080x1920 Instagram story images from selected sales, auto-uploads to Google Drive. **Asset Configurator (Nov 2024)**: Click-to-configure workflow on Generate Assets page. Main sale story and individual pick stories (all 1080x1920) support optional custom copy. All generated assets upload to Google Drive with Airtable tracking (FeaturedAssetURL/FeaturedAssetDate fields). **Background Job System (Nov 2024)**: Asset generation runs in background via `asset_jobs` PostgreSQL table, allowing users to navigate away without interrupting generation. Job status polled every 1 second, auto-navigates to results when complete. Configuration persisted per sale in `asset_configs` table with debounced auto-save. **Instagram Posting (Nov 2024)**: Integrated Late.dev API for programmatic Instagram posting. All assets post to Stories (feed posts deferred to future flow). Backend service (`server/instagram-poster.js`) supports single image posts, carousels, stories, and scheduled posts. Requires `LATE_API_KEY` and `LATE_INSTAGRAM_ACCOUNT_ID` secrets.
