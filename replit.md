...
- **Admin Interface**: Password-protected `/admin` panel using React Router with AdminLayout wrapper. Includes pages for managing product picks, sales approvals, brands, asset generation, freshness tracking, and Gem sync. Features a left sidebar navigation, session-based authentication via sessionStorage, and uniform auth header pattern across all API calls. Two-view pick manager, external link icons, delete confirmations, inline sale editing, and a price calculator dialog. Includes a "Add Brands" page with an editable results table.
